...
df_nervion <- df_monthly[df_monthly$Punto.de.Venta.Centro == df_monthly$Punto.de.Venta.Centro[188],]
df_nervion$dia <- as.character(str_sub(df_nervion$Fecha, 9,10))
...
df_nervion <- left_join(df_nervion, dfaux, by = "dia")
```

```{r}
df_nervion %>% group_by(Operacion, weekday, dia) %>% summarise(precioTicket = sum(Precio)) %>% 
  group_by(weekday) %>% summarise(count=n())
```

```{r}
m1 <- df_nervion %>% group_by(Operacion, weekday, dia, hora) %>% 
...
m11 %>% ggplot(aes(as.factor(hora), y = count)) +
  ...
  geom_text(aes(label = scales::percent(porcentaje), 
              ...
              vjust = -1) +
  ggtitle("Horas Puntas nervion en el Lunes")


#table(m1$hora)
#m1 %>% ggplot() +
#  geom_histogram(aes(as.factor(hora)), stat = "count", fill = "pink") +
#  ggtitle("Horas Puntas nervion en ls Lunes")
```


```{r}
m2 <- df_nervion %>% group_by(Operacion, weekday, dia, hora) %>% 
  summarise(precioTicket = sum(Precio)) %>% 
  filter(weekday==2) %>% 
  filter(hora>=0) 
...
m21 %>% ggplot(aes(as.factor(hora), y = count)) +
  ...
  geom_text(aes(label = scales::percent(porcentaje), 
              ...
              position = position_dodge(width = 0.9),
              vjust = -1)+
  ggtitle("Horas Puntas nervion en el Martes") 


#df_nervion %>% group_by(Operacion, a, dia, hora) %>% 
#  summarise(precioTicket = sum(Precio)) %>% 
#  filter(a==2) %>% 
#  filter(hora>=0) %>% 
#  ggplot() +
#  geom_histogram(aes(as.factor(hora)), stat = "count", fill = "pink") +
#  ggtitle("Horas Puntas nervion en ls Martes") 
```

```{r}
m3 <- df_nervion %>% group_by(Operacion, weekday, dia, hora) %>% 
  summarise(precioTicket = sum(Precio)) %>% 
  filter(weekday==3) %>% 
  filter(hora>=0) 
...
m31 %>% ggplot(aes(as.factor(hora), y = count)) +
  ...
  geom_text(aes(label = scales::percent(porcentaje), 
              ...
              position = position_dodge(width = 0.9),
              vjust = -1) +
  ggtitle("Horas Puntas nervion en el Miercoles")
```


```{r}
m4 <- df_nervion %>% group_by(Operacion, weekday, dia, hora) %>% 
  summarise(precioTicket = sum(Precio)) %>% 
  filter(weekday==4) %>% 
  filter(hora>=0) 
...
m41 %>% ggplot(aes(as.factor(hora), y = count)) +
  ...
  geom_text(aes(label = scales::percent(porcentaje), 
              ...
              position = position_dodge(width = 0.9),
              vjust = -1) +
  ggtitle("Horas Puntas nervion en el Jueves")
```

```{r}
m5 <- df_nervion %>% group_by(Operacion, weekday, dia, hora) %>% 
  summarise(precioTicket = sum(Precio)) %>% 
  filter(weekday==5) %>% 
  filter(hora>=0) 
...
m51 %>% ggplot(aes(as.factor(hora), y = count)) +
  ...
  geom_text(aes(label = scales::percent(porcentaje), 
              ...
              position = position_dodge(width = 0.9),
              vjust = -1) +
  ggtitle("Horas Puntas nervion en el Viernes")
```

```{r}
m6 <- df_nervion %>% group_by(Operacion, weekday, dia, hora) %>% 
  summarise(precioTicket = sum(Precio)) %>% 
  filter(weekday==6) %>% 
  filter(hora>=0) 
...
m61 %>% ggplot(aes(as.factor(hora), y = count)) +
  ...
  geom_text(aes(label = scales::percent(porcentaje), 
              ...
              position = position_dodge(width = 0.9),
              vjust = -1) +
  ggtitle("Horas Puntas nervion en el Sabado")
```
...
df_nervion9 <- df_monthly2[df_monthly2$Punto.de.Venta.Centro == df_monthly2$Punto.de.Venta.Centro[50],]
df_nervion9$dia <- as.character(str_sub(df_nervion9$Fecha, 9,10))
...
df_nervion9 <- left_join(df_nervion9, dfaux, by = "dia")
```

```{r}
df_nervion9 %>% group_by(Operacion, weekday, dia) %>% summarise(precioTicket = sum(Precio)) %>% 
  group_by(weekday) %>% summarise(count=n())
```

```{r}
m91 <- df_nervion9 %>% group_by(Operacion, weekday, dia, hora) %>% 
  summarise(precioTicket = sum(Precio)) %>% 
  filter(weekday==1) %>% 
  filter(hora>=0) 

m92 <- df_nervion9 %>% group_by(Operacion, weekday, dia, hora) %>% 
  summarise(precioTicket = sum(Precio)) %>% 
  filter(weekday==2) %>% 
  filter(hora>=0) 

m93 <- df_nervion9 %>% group_by(Operacion, weekday, dia, hora) %>% 
  summarise(precioTicket = sum(Precio)) %>% 
  filter(weekday==3) %>% 
  filter(hora>=0) 

m94 <- df_nervion9 %>% group_by(Operacion, weekday, dia, hora) %>% 
  summarise(precioTicket = sum(Precio)) %>% 
  filter(weekday==4) %>% 
  filter(hora>=0) 

m95 <- df_nervion9 %>% group_by(Operacion, weekday, dia, hora) %>% 
  summarise(precioTicket = sum(Precio)) %>% 
  filter(weekday==5) %>% 
  filter(hora>=0) 

m96 <- df_nervion9 %>% group_by(Operacion, weekday, dia, hora) %>% 
  summarise(precioTicket = sum(Precio)) %>% 
  filter(weekday==6) %>% 
  filter(hora>=0) 
```
...
```{r}
nervion <- rbind(df_nervion, df_nervion9)
nervion$mes <- str_sub(nervion$Fecha, 6,7)
```


```{r}
nervion1 <- nervion %>% group_by(Operacion, weekday, dia, hora, mes) %>% 
 summarise(precioTicket = sum(Precio)) %>% 
 filter(weekday==1) %>% 
 filter(hora>=0)
...
```{r}
nervion2 <- nervion %>% group_by(Operacion, weekday, dia, hora, mes) %>% 
  summarise(precioTicket = sum(Precio)) %>% 
  filter(weekday==2) %>% 
  filter(hora>=0) 
...
```{r}
nervion3 <- nervion %>% group_by(Operacion, weekday, dia, hora, mes) %>% 
  summarise(precioTicket = sum(Precio)) %>% 
  filter(weekday==3) %>% 
  filter(hora>=0) 
...
```{r}
nervion4 <- nervion %>% group_by(Operacion, weekday, dia, hora, mes) %>% 
  summarise(precioTicket = sum(Precio)) %>% 
  filter(weekday==4) %>% 
  filter(hora>=0) 
...
```{r}
nervion5 <- nervion %>% group_by(Operacion, weekday, dia, hora, mes) %>% 
  summarise(precioTicket = sum(Precio)) %>% 
  filter(weekday==5) %>% 
  filter(hora>=0) 
...
```{r}
nervion6 <- nervion %>% group_by(Operacion, weekday, dia, hora, mes) %>% 
  summarise(precioTicket = sum(Precio)) %>% 
  filter(weekday==6) %>% 
  filter(hora>=0) 
