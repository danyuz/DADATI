...
df_goya <- df_monthly[df_monthly$Punto.de.Venta.Centro == df_monthly$Punto.de.Venta.Centro[5],]
df_goya$dia <- as.character(str_sub(df_goya$Fecha, 9,10))
...
df_goya <- left_join(df_goya, dfaux, by = "dia")
```

```{r}
df_goya %>% group_by(Operacion, weekday, dia) %>% summarise(precioTicket = sum(Precio)) %>% 
  group_by(weekday) %>% summarise(count=n())
```

```{r}
m1 <- df_goya %>% group_by(Operacion, weekday, dia, hora) %>% 
...
m11 %>% ggplot(aes(as.factor(hora), y = count)) +
  ...
  geom_text(aes(label = scales::percent(porcentaje), 
              ...
              vjust = -1) +
  ggtitle("Horas Puntas goya en ls Lunes")
...
```{r}
m2 <- df_goya %>% group_by(Operacion, weekday, dia, hora) %>% 
  summarise(precioTicket = sum(Precio)) %>% 
  filter(weekday==2) %>% 
  filter(hora>=0) 
...
m21 %>% ggplot(aes(as.factor(hora), y = count)) +
  ...
  geom_text(aes(label = scales::percent(porcentaje), 
              ...
              position = position_dodge(width = 0.9),
              vjust = -1)+
  ggtitle("Horas Puntas goya en ls Martes") 


#df_goya %>% group_by(Operacion, a, dia, hora) %>% 
#  summarise(precioTicket = sum(Precio)) %>% 
#  filter(a==2) %>% 
#  filter(hora>=0) %>% 
#  ggplot() +
...
```{r}
m3 <- df_goya %>% group_by(Operacion, weekday, dia, hora) %>% 
  summarise(precioTicket = sum(Precio)) %>% 
  filter(weekday==3) %>% 
  filter(hora>=0) 
...
m31 %>% ggplot(aes(as.factor(hora), y = count)) +
  ...
  geom_text(aes(label = scales::percent(porcentaje), 
              ...
              position = position_dodge(width = 0.9),
              vjust = -1) +
  ggtitle("Horas Puntas goya en ls Miercoles")
```


```{r}
m4 <- df_goya %>% group_by(Operacion, weekday, dia, hora) %>% 
  summarise(precioTicket = sum(Precio)) %>% 
  filter(weekday==4) %>% 
  filter(hora>=0) 
...
m41 %>% ggplot(aes(as.factor(hora), y = count)) +
  ...
  geom_text(aes(label = scales::percent(porcentaje), 
              ...
              position = position_dodge(width = 0.9),
              vjust = -1) +
  ggtitle("Horas Puntas goya en ls Jueves")
```

```{r}
m5 <- df_goya %>% group_by(Operacion, weekday, dia, hora) %>% 
  summarise(precioTicket = sum(Precio)) %>% 
  filter(weekday==5) %>% 
  filter(hora>=0) 
...
m51 %>% ggplot(aes(as.factor(hora), y = count)) +
  ...
  geom_text(aes(label = scales::percent(porcentaje), 
              ...
              position = position_dodge(width = 0.9),
              vjust = -1) +
  ggtitle("Horas Puntas goya en ls Viernes")
```

```{r}
m6 <- df_goya %>% group_by(Operacion, weekday, dia, hora) %>% 
  summarise(precioTicket = sum(Precio)) %>% 
  filter(weekday==6) %>% 
  filter(hora>=0) 
...
m61 %>% ggplot(aes(as.factor(hora), y = count)) +
  ...
  geom_text(aes(label = scales::percent(porcentaje), 
              ...
              position = position_dodge(width = 0.9),
              vjust = -1) +
  ggtitle("Horas Puntas goya en ls Sabado")
```
...
df_goya9 <- df_monthly2[df_monthly2$Punto.de.Venta.Centro == df_monthly2$Punto.de.Venta.Centro[1],]
df_goya9$dia <- as.character(str_sub(df_goya9$Fecha, 9,10))
...
df_goya9 <- left_join(df_goya9, dfaux, by = "dia")
```

```{r}
df_goya9 %>% group_by(Operacion, weekday, dia) %>% summarise(precioTicket = sum(Precio)) %>% 
  group_by(weekday) %>% summarise(count=n())
```

```{r}
m91 <- df_goya9 %>% group_by(Operacion, weekday, dia, hora) %>% 
  summarise(precioTicket = sum(Precio)) %>% 
  filter(weekday==1) %>% 
  filter(hora>=0) 

m92 <- df_goya9 %>% group_by(Operacion, weekday, dia, hora) %>% 
  summarise(precioTicket = sum(Precio)) %>% 
  filter(weekday==2) %>% 
  filter(hora>=0) 

m93 <- df_goya9 %>% group_by(Operacion, weekday, dia, hora) %>% 
  summarise(precioTicket = sum(Precio)) %>% 
  filter(weekday==3) %>% 
  filter(hora>=0) 

m94 <- df_goya9 %>% group_by(Operacion, weekday, dia, hora) %>% 
  summarise(precioTicket = sum(Precio)) %>% 
  filter(weekday==4) %>% 
  filter(hora>=0) 

m95 <- df_goya9 %>% group_by(Operacion, weekday, dia, hora) %>% 
  summarise(precioTicket = sum(Precio)) %>% 
  filter(weekday==5) %>% 
  filter(hora>=0) 

m96 <- df_goya9 %>% group_by(Operacion, weekday, dia, hora) %>% 
  summarise(precioTicket = sum(Precio)) %>% 
  filter(weekday==6) %>% 
  filter(hora>=0) 
```
...
```{r}
goya <- rbind(df_goya, df_goya9)
goya$mes <- str_sub(goya$Fecha, 6,7)
```


```{r}
goya1 <- goya %>% group_by(Operacion, weekday, dia, hora, mes) %>% 
 summarise(precioTicket = sum(Precio)) %>% 
 filter(weekday==1) %>% 
 filter(hora>=0)
...
goya1 %>% ggplot() +
  ...
  ggtitle("Horas Puntas goya en el Lunes")+ 
  labs(x = "Hora", y = "Count") +
  facet_wrap(~mes)
```
...
```{r}
goya2 <- goya %>% group_by(Operacion, weekday, dia, hora, mes) %>% 
  summarise(precioTicket = sum(Precio)) %>% 
  filter(weekday==2) %>% 
  filter(hora>=0) 
...
goya2 %>% ggplot() +
  ...
  facet_grid(~mes)+
  labs(x = "Hora", y = "Count") +
  ggtitle("Horas Puntas goya en el Martes")
```

```{r}
goya3 <- goya %>% group_by(Operacion, weekday, dia, hora, mes) %>% 
  summarise(precioTicket = sum(Precio)) %>% 
  filter(weekday==3) %>% 
  filter(hora>=0) 
...
goya3 %>% ggplot() +
  ...
  facet_grid(~mes) +
  labs(x = "Hora", y = "Count") +
  ggtitle("Horas Puntas goya en el Miercoles")
```



```{r}
goya4 <- goya %>% group_by(Operacion, weekday, dia, hora, mes) %>% 
  summarise(precioTicket = sum(Precio)) %>% 
  filter(weekday==4) %>% 
  filter(hora>=0) 
...
goya4 %>% ggplot() +
  ...
  facet_grid(~mes) +
  labs(x = "Hora", y = "Count") +
  ggtitle("Horas Puntas goya en el Jueves")
```


```{r}
goya5 <- goya %>% group_by(Operacion, weekday, dia, hora, mes) %>% 
  summarise(precioTicket = sum(Precio)) %>% 
  filter(weekday==5) %>% 
  filter(hora>=0) 
...
goya5 %>% ggplot() +
  ...
  facet_grid(~mes) +
  labs(x = "Hora", y = "Count") +
  ggtitle("Horas Puntas goya en el Viernes")
```



```{r}
goya6 <- goya %>% group_by(Operacion, weekday, dia, hora, mes) %>% 
  summarise(precioTicket = sum(Precio)) %>% 
  filter(weekday==6) %>% 
  filter(hora>=0) 
...
goya6 %>% ggplot() +
  ...
  labs(x = "Hora", y = "Count") +
  facet_grid(~mes) +
  ggtitle("Horas Puntas goya en el Sabado")
```
