...
ESPA <- df %>% filter(str_detect(cliente, "ESPA"))
...
```{r}
aux_mayorista <- c(unique(filter(mayorista, str_detect(cliente, "DECOGLOBAL"))$cliente), unique(filter(mayorista, str_detect(cliente, "OSCAR"))$cliente), unique(filter(mayorista, str_detect(cliente, "DELFIN"))$cliente), unique(filter(mayorista, str_detect(cliente, "SIGASCORES"))$cliente))

mayorista_tabla1 <- mayorista %>% mutate(cliente = replace(cliente, !str_detect(cliente, "DECOGLOBAL|OSCAR|DELFIN|SIGASCORES"), "Mayorista_General"))
...
df1$Porcentaje_por_canal <- round(df1$vental_total_por_canal/total,4)
...
df2$porcentaje_porVenta <- round(df2$total_Por_Cliente/total,4)
...
df4$porcentaje<- round(df4$total_por_Cliente_nacional/total_nacional,4)
...
df5$porcentaje<- round(df5$total_por_Cliente_Internacional/total,4)
